...
ofstream ofile2;
ifstream ifile2;
double const pi_ = acos(-1);
double ANALYTICAL2 = 5.0 * pi_ * pi_ / (16.0*16.0); // Setting the analytical solution
...
void legendre(double a, double b, int steps, string file) {
    ...
    if (ifile2.peek() == EOF) {
        ofile2 << "steps, exact, numeric, error, variance, time" << endl;
    } // End if

    ofile2 << setw(1) << setprecision(3) <<  steps     << ", ";
    ofile2 << setw(1) <<   ANALYTICAL2 	<<           ", ";
    ofile2 << setw(1) << setprecision(3) <<  integral_segments   << ", ";
