...
double function_integrand(double r_1, double r_2, double theta_1, double theta_2, double phi_1, double phi_2){
    ...
    double r12 = r_1*r_1 + r_2*r_2 - 2*r_1*r_2*cos_beta;

    if (r12 < 1E-9) return 0.0;
    else return (exp(-3.0*(r_1 + r_2))*sin(theta_1)*sin(theta_2))/(sqrt(r12));
} // End function_integrand
...
void laguerre(double a, double b, int steps, string file){
    ...
    ofile_lag.open(file, ios::app); ifile_lag.open(file); // File is read as input argument

    ofile_lag << setiosflags(ios::showpoint | ios::uppercase);

    /* This if-statement checks if the file is empty
     * That way, it will not not have to print out the labels each time */
    if (ifile_lag.peek() == EOF) {
        ofile_lag << "steps, exact, numeric, error, time" << endl;
    } // End if

    ofile_lag << setw(1) << setprecision(3) <<  steps << ", ";
    ofile_lag << setw(1) <<   ANALYTICAL 	<<           ", ";
    ofile_lag << setw(1) << setprecision(3) <<  sum   << ", ";
    ofile_lag << setw(1) << setprecision(3) <<  error << ", ";
    ofile_lag << setw(1) << setprecision(3) <<  time  << endl;
