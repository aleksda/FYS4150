...
int main() {
	...
	int a= -5: int b = 5, int n = 1e6; string file = 'test.csv';
...
#pragma omp parallel for reduction(+:cmc) reduction(+:sigma) private(i)
    for(i = 0; i < n; i++) {

        u 		= -log(1-ran0(&idum));
        v 		= -log(1-ran0(&idum));
        theta1 	= M_PI * ran0(&idum);
        theta2 	= M_PI * ran0(&idum);
        phi1 	= 2 * M_PI * ran0(&idum);
        phi2 	= 2 * M_PI * ran0(&idum);
	 ...
	 * Also this allows us to easily update our datafiles */
    if (filein.peek() == EOF) {
